...
int main() {
  ...
  data = stbi_load("../resources/textures/peepo.jpg", &height, &width,
                   &nrChannels, 0);
  if (data) {
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, height, width, 0, GL_RGB,
                 GL_UNSIGNED_BYTE, data);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    for (unsigned int i = 0; i < 10; i++) {
      ...
      float angle = 20.0f * i;
      model =
          glm::rotate(model, (float)glfwGetTime() * ((i / 2) + 1),
                      glm::vec3(1.0f, 0.3f, glm::sin((float)glfwGetTime())) +
                          cubePositions[i]);
    ...
    view = glm::rotate(view, (float)glfwGetTime(),
                       glm::vec3(0.0f, 1.0f, (float)glm::sin(glfwGetTime())));
